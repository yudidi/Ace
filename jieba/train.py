...
print("读取停止词...")
stopwords = {line.strip() for line in open('stopwords.txt').readlines()}  # 读取停止词文件并保存到列表stopwords
# for ts in stopwords:
#   print ts
print("停止词读取完毕")
...
if __name__ == '__main__':
            ...
            for ssss in sss:
                ...
                for r in result:
                    if r not in stopwords and r != " " and r != '\n' and r != "":
                        if r not in words:
                            words[r] = WORD()
                        if category not in words[r].incategorys:
                            words[r].incategorys[category] = 0
                        words[r].count += 1
                        if r not in categorys[category].inwords:
    ...
    print("\n训练结束，正在保存...")
    pk.dump(words, f_parameter_words)
    pk.dump(categorys, f_parameter_categorys)
    print("保存完毕")
